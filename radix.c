...
static const uint64_t masks[6] = { l0_mask, l1_mask, l2_mask, l3_mask, l4_mask, l5_mask };

static
rdxnode_t get_node(rdxtree_t tree, diskptr_t ptr)
{
...
static
rdxnode_t create_node(rdxtree_t tree, int isleaf)
{
  struct buf *bp = alloc_blk(BLKSZ);
  rdxnode_t node = malloc(sizeof(rdxnode));
  node->rdx_bp = bp;
...
static inline
int leaf_matches(uint64_t key, rdx_node_header *header)
{
  uint64_t mask = 0;
  for (uint8_t i = 0; i < header->prefix_len; i++) {
      mask |= ((uint64_t) header->prefix_arr[i]) << ((MAX_ARR - i) * RADIX_BIT_WIDTH);
  }
  return (mask & key) == mask;
}

static inline
int check_prefix(uint64_t key, rdx_node_header *header)
{
  uint64_t mask = 0;

  int num_matches = 0;
  for (uint8_t i = 0; i < header->prefix_len; i++) {
      mask = ((uint64_t) header->prefix_arr[i]) << ((MAX_ARR - i - header->depth) * RADIX_BIT_WIDTH);
      if ((mask & key) == mask) {
        num_matches += 1;
      }
  }
